...
cmap = {
    ...
    'Electricity_Cost_Low': 'lightgreen',  # Retained but unused
    'Electricity_Cost_High': 'darkgreen'   # Retained but unused
}

# Read route sheets from Excel
route_cat = {}
for route in Routes:
    try:
...
# Compute route-level cost data 
# Core modification: Replace low/mean/high electricity cost with 2030/2045/2060 values
data_route_cost = []
for route in Routes:
    ...
    # Extract electricity cost for 2030/2045/2060 (mean of provinces)
    Electricity_Cost_2030 = df[2030].mean() if 2030 in df.columns else 0.0
    Electricity_Cost_2045 = df[2045].mean() if 2045 in df.columns else 0.0
    Electricity_Cost_2060 = df[2060].mean() if 2060 in df.columns else 0.0
    ...
    data_route_cost.append({
        ...
        'Electricity_Cost_2030': Electricity_Cost_2030,  # Add 2030 electricity cost
        'Electricity_Cost_2045': Electricity_Cost_2045,  # Add 2045 electricity cost
        'Electricity_Cost_2060': Electricity_Cost_2060,  # Add 2060 electricity cost
        'CCS_Cost': CCS_Cost,
...
# Plot a 
# Core modification: Replace bar meaning with 2030/2045/2060 costs
ax1 = plt.subplot(grid[:20, :20])
index_1 = np.arange(len(data_route_cost)) - 0.3  # Bar position for 2030
index_2 = np.arange(len(data_route_cost))        # Bar position for 2045
index_3 = np.arange(len(data_route_cost)) + 0.3  # Bar position for 2060
width = 0.25
# Modify x-axis labels: Low/Mean/High → 2030/2045/2060
cost_labels = ['2030       2045       2060\n' + r for r in data_route_cost['Route'].tolist()]

stack_categories = ['Fixed_Cost', 'Operating_Cost', 'Fuel_Cost', 'Material_Cost', 'Electricity_Cost', 'CCS_Cost', 'Transportation_Cost']
bottom1 = np.zeros(len(data_route_cost))  # Stack bottom for 2030 bars
bottom2 = np.zeros(len(data_route_cost))  # Stack bottom for 2045 bars
bottom3 = np.zeros(len(data_route_cost))  # Stack bottom for 2060 bars

for cat in stack_categories:
    if cat == 'Electricity_Cost':
        # Electricity cost: 3 bars use 2030/2045/2060 values
        plt.bar(index_1, data_route_cost['Electricity_Cost_2030'], bottom=bottom1, width=width, 
                color=cmap['Electricity_Cost'], alpha=0.8, edgecolor='k')
        plt.bar(index_2, data_route_cost['Electricity_Cost_2045'], bottom=bottom2, width=width, 
                color=cmap['Electricity_Cost'], alpha=0.8, edgecolor='k')
        plt.bar(index_3, data_route_cost['Electricity_Cost_2060'], bottom=bottom3, width=width, 
                label='Electricity_Cost', color=cmap['Electricity_Cost'], alpha=0.8, edgecolor='k')
        # Update stack bottom
        bottom1 += data_route_cost['Electricity_Cost_2030'].values
        bottom2 += data_route_cost['Electricity_Cost_2045'].values
        bottom3 += data_route_cost['Electricity_Cost_2060'].values
    else:
        # Other costs: Same value for 3 years (mean value)
...
# Draw carbon price bands (logic unchanged)
...
plt.bar(index_2, 100*carbon_emission, bottom=bottom2, width=width, label='Carbon Price (100 CNY/tCO2e)', color='white', alpha=0.8, edgecolor='k', linestyle='--', hatch='//')
...
plt.bar(index_2, 400*carbon_emission, bottom=bottom2, width=width, label='Carbon Price (500 CNY/tCO2e)', color='white', alpha=0.8, edgecolor='k', linestyle='--', hatch='..')
...
plt.bar(index_2, 500*carbon_emission, bottom=bottom2, width=width, label='Carbon Price (1000 CNY/tCO2e)', color='white', alpha=0.8, edgecolor='k', linestyle='--', hatch='xx')
...
# Plot b (H2-DRI-EAF in 2030, sorted by total cost)
...
# Plot c (H2-DRI-EAF in 2060, sorted by total cost)
if route_for_prov in route_cat:
...
# Plot d (BF-BOF-CCS in 2030, sorted by total cost)
...
# Plot e (BF-BOF-CCS in 2060, sorted by total cost)
if route_ccs in route_cat:
...
# Save figures
plt.savefig("figs/FigS3.png", dpi=600, transparent=True)
plt.savefig("figs/FigS3.svg", dpi=600, transparent=True)