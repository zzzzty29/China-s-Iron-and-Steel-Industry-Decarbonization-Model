...
electricity_cost_multipliers = [-0.2, -0.15, -0.1, -0.05, 0.0, 0.05, 0.1, 0.15, 0.2]
cpp_shares = [0.0, 0.25, 0.5, 0.75, 1.0]
...
total_iterations = (
    len(carbon_modes)
    * len(material_cost_multipliers)
    * len(electricity_cost_multipliers)
    * len(cpp_shares)
    * len(phaseout_factors)
    * len(elasticity_multipliers)
)
current_iteration = 0
...
for carbon_mode in carbon_modes:
    ...
    # Read material cost data & electricity cost data
    ...
    for material_multiplier in material_cost_multipliers:
        for electricity_multiplier in electricity_cost_multipliers:
            for cpp_share in cpp_shares:
                ...
                for year in years:
                    if year in eaf_material.columns:
                        # ---- Material cost sensitivity adjustment ----
                        ...
                        '''
                        if np.any(ccs_cost[year] < 0):
                            print("Problem year:", year)
                            print("ccs_cost_original:", ccs_cost_original[year])
                            print("ccs_cost:", ccs_cost[year])
                            print("material_multiplier:", material_multiplier)
                            print("ccs_material_cost:", ccs_material['Material_Cost'])
                            raise ValueError(
                                f"Negative CCS cost detected in year {year}. "
                                f"Min value: {ccs_cost[year]}, "
                            )'''

                        # ---- Electricity cost sensitivity adjustment ----

                        # grid electricity price
                        P_grid = data_province[year].values
                        P_grid_sens = P_grid * (1 + electricity_multiplier)

                        # CPP electricity price
                        P_cpp = data_province["CPP electricity(CNY/kWh)"].values

                        # ===== 1. EAF / H2 =====
                        ratio_other = P_grid_sens / P_grid

                        eaf_cost[year] = eaf_cost[year] + eaf_material[year] * (ratio_other - 1)
                        h2_cost[year]  = h2_cost[year]  + h2_material[year]  * (ratio_other - 1)

                        # ===== 2. BF / CCS =====
                        P_new = cpp_share * P_cpp + (1 - cpp_share) * P_grid_sens

                        ratio_bf = P_new / P_grid

                        bf_cost[year] = bf_cost[year] + bf_material[year] * (ratio_bf - 1)
                        ccs_cost[year] = ccs_cost[year] + ccs_material[year] * (ratio_bf - 1)
                        '''
                        # ---- Safety check: CCS cost must be strictly positive ----
                        if np.any(ccs_cost[year] < 0):
                            print(P_grid_sens, P_new, P_cpp, P_grid, ratio_bf, ratio_other)
                            raise ValueError(
                                f"Negative CCS cost detected in year {year}. "
                                f"Min value: {ccs_cost[year]}, "
                                f"cpp_share={cpp_share}, electricity_multiplier={electricity_multiplier}"
                            )'''
